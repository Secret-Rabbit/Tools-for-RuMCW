...
for item in redirects:
    ...
    title = f"Файл:{item}"
    redirectTarget = f"Файл:{redirects[item]}"
    ...
    if page.exists():
        ...
        # Избежание двойных перенаправлений
        if targetPage.isRedirectPage():
            # content: содержимое новой страницы.
            print(f"🔁 Страница '{redirectTarget}' является перенаправлением. Будет создано перенаправление по цели перенаправления.")
